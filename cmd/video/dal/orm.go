...
func MGetVideoByUserID(ctx context.Context, userID int64) ([]*model.Video, error) {
	...
	if err := DB.WithContext(ctx).Preload("Videos").Find(&user).Error; err != nil {
		return nil, err
	}
...
func GetLikeCount(ctx context.Context, videoID int64) (int64, error) {
	...
	return DB.WithContext(ctx).Model(&video).Association("Likes").Count(), nil
}

// GetCommentCount 返回视频评论数
func GetCommentCount(ctx context.Context, videoID int64) (int64, error) {
	video := model.Video{VideoID: uint(videoID)}
	return DB.WithContext(ctx).Model(&video).Association("Comments").Count(), nil
}

// IsFavorite 返回是否点赞
func IsFavorite(ctx context.Context, videoID int64, userID int64) (bool, error) {
	user := model.User{UserID: uint(userID)}
	return DB.WithContext(ctx).Model(&user).Where("`like`.video_id = ?", videoID).Association("Likes").Count() > 0, nil
}
...
func LikeVideo(ctx context.Context, userID int64, videoID int64) error {
	user := model.User{UserID: uint(userID)}
	video := model.Video{VideoID: uint(videoID)}
	return DB.WithContext(ctx).Model(&user).Association("Likes").Append(&video)
}
...
func UnLikeVideo(ctx context.Context, userID int64, videoID int64) error {
	user := model.User{UserID: uint(userID)}
	video := model.Video{VideoID: uint(videoID)}
	return DB.WithContext(ctx).Model(&user).Association("Likes").Delete(&video)
}

func MGetLikeVideo(ctx context.Context, userID int64) ([]*model.Video, error) {
	user := model.User{UserID: uint(userID)}
	if err := DB.WithContext(ctx).Preload("Likes").Find(&user).Error; err != nil {
		return nil, err
	}
...
func DeleteComment(ctx context.Context, commentID int64) error {
	return DB.WithContext(ctx).Delete(&model.Comment{}, commentID).Error
}

// MGetComment 查询评论,需要dal层返回评论详情,有可能有多条评论
func MGetComment(ctx context.Context, videoID int64) ([]*model.Comment, error) {
	video := model.Video{VideoID: uint(videoID)}
	if err := DB.WithContext(ctx).Preload("Comments").Find(&video).Error; err != nil {
		return nil, err
	}
